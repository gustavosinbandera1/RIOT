...
static void _send(gnrc_pktsnip_t *pkt)
{
    ...
    gnrc_nettype_t target_type = pkt->type;
     ...
     DEBUG("EL TAMANO DEL PAQUETE ESD %d\n",gnrc_pkt_len(pkt));
     DEBUG("COUNT ES %d",gnrc_pkt_count(pkt));
    ...
    pkt = tmp;
    udp_snip = tmp->next;
  DEBUG("EL TIPO ES (pkt->type)  %d",pkt->type);//netif

  DEBUG("EL TIPO ES (udp_snip->type) %d",udp_snip->type);//ipv6
  ...
  DEBUG("EL TIPO ES (temp->type) %d",tmp->type);//ipv6
  DEBUG("EL TIPO ES (tmp->next->type)  %d",tmp->next->type);//udp
