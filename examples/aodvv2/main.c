...
/* IPv6 header + payload:https://github.com/gustavosinbandera1/RIOT     version+TC  FL: 0       plen: 16    NH:17 HL:64 */
...
(void)argv;
    ...
    (void)neighbor;
    (void)_nbr_mac;
   // inet_pton(AF_INET6, argv[1], &neighbor); //converts an IPv4 or IPv6 Internet network address in its standard text presentation form into its numeric binary form.
...
inet_pton(AF_INET6, argv[1], &neighbor); 
    ...
    //                             _nbr_mac, sizeof(_nbr_mac));
     res = gnrc_ipv6_nib_nc_set(&neighbor, _mock_netif->pid,
                                 (uint8_t*)&neighbor.u8[16], sizeof(neighbor.u8[14]));
    (void)res;
  //  printf("agregando neigh:  %d", res);
...
int demo_attempt_to_send(char* dest_str, char* msg)
{
    ...
    while(num_attempts < DISCOVERY_ATTEMPTS_MAX) {
        int bytes_sent = sendto(_sock_snd, msg, msg_len,
                                                0, (struct sockaddr*)&_sockaddr, sizeof _sockaddr);
...
static void *_demo_receiver_thread(void *arg)
{
    ...
    if (-1 == bind(sock_rcv,(struct sockaddr*)&sa_rcv.sin6_addr, sizeof(sa_rcv.sin6_addr))) {
